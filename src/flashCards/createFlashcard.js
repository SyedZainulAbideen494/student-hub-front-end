...
const performCalculate = async () => {
  ...
  try {
    ...
    // On success, set the results and clear the query
    setResults([{ title: 'Result', content: formattedContent || 'No content available' }]);
    setQuery(''); // Clear the input after successful calculation
  } catch (error) {
    console.error('Calculation Error:', error);
    // On failure, keep the query and show the error
    setResults([{ title: 'Error', content: "We're sorry, something went wrong. Please try again." }]);
  } finally {
    setLoading(false);
  }
};

// Function to handle regeneration using the same query
const handleRegenerate = async () => {
  await performCalculate(); // Reuse the same query to trigger the calculation again
};
...
<div className="form-group-flashcards-page">
    ...
    <div className="button-container__generate__text__flashcard__create">
      <button
        className="button__generate__text__flashcard__create"
        type="button"
      ...
      >
        ...
        <span className="text_button__generate__text__flashcard__create">
          {loading ? 'Generating' : 'Generate Info'}
        </span>
      </button>

      {/* Regenerate button shown only when there's an error */}
      {results.length > 0 && results[0].title === 'Error' && (
        <div className="button-container__regenerate__text__flashcard__create">
          <button
            className="button__regenerate__text__flashcard__create"
            type="button"
